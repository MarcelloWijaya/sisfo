...
@section('title', __('all.edit_schedule'))
@section('header', __('all.edit_schedule'))
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <form action="{{ route('classrooms.update', $classroom) }}" method="POST">
                ...
                <div class="p-6 space-y-5">
                            ...
                            <select name="branch_id" class="w-full px-3 py-2 border rounded-lg focus:ring-[#90C74A]" required>
                                ...
                                @foreach ($branches as $branch)
                                    <option value="{{ $branch->id }}"
                                        {{ old('branch_id', $classroom->branch_id) == $branch->id ? 'selected' : '' }}>
                                        {{ $branch->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    @else
                        <input type="hidden" name="branch_id" value="{{ $classroom->branch_id }}">
                    @endif
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <select name="day" class="w-full px-3 py-2 border rounded-lg focus:ring-[#90C74A]" required>
                                ...
                                @foreach ($days as $key => $day)
                                    <option value="{{ $key }}"
                                        {{ old('day', $classroom->day) == $key ? 'selected' : '' }}>{{ __("all.$key") }}
                                    </option>
                                @endforeach
                            </select>
                            @error('day')
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('all.room') }} <span
                                    class="text-red-500">*</span></label>
                            <input type="text" name="room" value="{{ old('room', $classroom->room) }}" required
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('all.start_time') }} <span
                                    class="text-red-500">*</span></label>
                            <input type="time" name="start_time" value="{{ old('start_time', $classroom->start_time) }}"
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('all.end_time') }} <span
                                    class="text-red-500">*</span></label>
                            <input type="time" name="end_time" value="{{ old('end_time', $classroom->end_time) }}"
                                required class="w-full px-3 py-2 border rounded-lg focus:ring-[#90C74A]">
                            @error('end_time')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <select name="level" class="w-full px-3 py-2 border rounded-lg focus:ring-[#90C74A]" required>
                                ...
                                @foreach ($levels as $key => $level)
                                    <option value="{{ $key }}"
                                        {{ old('level', $classroom->level) == $key ? 'selected' : '' }}>
                                        {{ __("all.$key") }}</option>
                                @endforeach
                            </select>
                            @error('level')
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('all.activity') }} <span
                                class="text-red-500">*</span></label>
                        <input type="text" name="activity" value="{{ old('activity', $classroom->activity) }}" required
                    ...
                    <div>
                        ...
                        <input type="number" name="quota" value="{{ old('quota', $classroom->quota) }}" min="1"
                            class="w-full px-3 py-2 border rounded-lg focus:ring-[#90C74A]">
                        @error('quota')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
