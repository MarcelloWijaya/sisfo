...
<aside class="w-72 bg-white shadow-xl hidden md:block fixed h-full overflow-y-auto z-30">
    ...
    <nav class="p-4 space-y-1">
        ...
        <a href="{{ url('/schedules') }}"
            class="flex items-center gap-3 px-4 py-3 rounded-xl text-gray-600 hover:bg-[#90C74A] hover:text-white transition-all duration-300 group {{ request()->is('schedules*') ? 'bg-[#90C74A] text-white' : '' }}">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
            </svg>
            <span>{{ __('all.schedules') }}</span>
        </a>

        <a href="{{ url('/attendance') }}"
            class="flex items-center gap-3 px-4 py-3 rounded-xl text-gray-600 hover:bg-[#90C74A] hover:text-white transition-all duration-300 group {{ request()->is('attendance*') ? 'bg-[#90C74A] text-white' : '' }}">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
            </svg>
            <span>{{ __('all.attendance') }}</span>
        </a>
