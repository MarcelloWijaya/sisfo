...
@section('title', __('all.edit_teacher_attendance'))
@section('header', __('all.edit_teacher_attendance'))

@section('content')
    <div class="max-w-2xl mx-auto">
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <form action="{{ route('attendance.teacher.update', $teacherAttendance) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="p-6 space-y-5">
                            ...
                            <select name="branch_id" class="w-full px-3 py-2 border rounded-lg bg-gray-100" disabled>
                                @foreach ($branches as $branch)
                                    <option value="{{ $branch->id }}"
                                        {{ $teacherAttendance->branch_id == $branch->id ? 'selected' : '' }}>
                                        {{ $branch->name }}
                                    </option>
                                @endforeach
                            </select>
                            <input type="hidden" name="branch_id" value="{{ $teacherAttendance->branch_id }}">
                        </div>
                    @else
                        <input type="hidden" name="branch_id" value="{{ $teacherAttendance->branch_id }}">
                    @endif
                    ...
                    <div>
                        ...
                        <select name="teacher_id" class="w-full px-3 py-2 border rounded-lg focus:ring-[#90C74A]" required>
                            <option value="">{{ __('all.select_teacher') }}</option>
                            @foreach ($teachers as $teacher)
                                <option value="{{ $teacher->id }}"
                                    {{ old('teacher_id', $teacherAttendance->teacher_id) == $teacher->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <input type="date" name="attendance_date"
                            value="{{ old('attendance_date', $teacherAttendance->attendance_date) }}" required
                        ...
                        <div>
                            ...
                            <select name="status" class="w-full px-3 py-2 border rounded-lg focus:ring-[#90C74A]" required>
                                @foreach ($statuses as $key => $status)
                                    <option value="{{ $key }}"
                                        {{ old('status', $teacherAttendance->status) == $key ? 'selected' : '' }}>
                                        {{ __("all.$key") }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div>
                        ...
                        <textarea name="notes" rows="3" class="w-full px-3 py-2 border rounded-lg focus:ring-[#90C74A]"
                            placeholder="{{ __('all.notes_placeholder') }}">{{ old('notes', $teacherAttendance->notes) }}</textarea>
                    </div>
                </div>
