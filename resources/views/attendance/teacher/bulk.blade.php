...
@section('content')
    <div class="max-w-4xl mx-auto">
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <form action="{{ route('attendance.teacher.bulk.store') }}" method="POST">
                @csrf
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-100">
                                @forelse ($teachers as $index => $teacher)
                                    <tr>
                                        <td class="px-4 py-3 font-medium">{{ $teacher->full_name }}</td>
                                        <td class="px-4 py-3 text-sm">{{ $teacher->teacher_id }}</td>
                                        <td class="px-4 py-3">
                                            <select name="attendances[{{ $index }}][status]"
                                                ...
                                                <option value="sick">{{ __('all.sick') }}</option>
                                                <option value="leave">{{ __('all.leave') }}</option>
                                            </select>
                                            <input type="hidden" name="attendances[{{ $index }}][teacher_id]"
                                                value="{{ $teacher->id }}">
                                        </td>
                                        <td class="px-4 py-3">
