...
@section('title', __('all.student_attendance'))
@section('header', __('all.student_attendance'))
    ...
    <div class="space-y-6">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                ...
                <p class="text-sm text-gray-500">{{ __('all.manage_student_attendance') }}</p>
            </div>
            <div class="flex gap-3">
                <a href="{{ route('attendance.student.bulk.create') }}?schedule_id={{ request('schedule_id') }}&attendance_date={{ request('date', date('Y-m-d')) }}"
                ...
                <a href="{{ route('attendance.student.create') }}"
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
            <form method="GET" class="flex flex-col md:flex-row gap-3">
                ...
                <select name="student_id" class="px-4 py-2.5 border border-gray-300 rounded-xl">
                    <option value="">{{ __('all.all_students') }}</option>
                ...
                <select name="schedule_id" class="px-4 py-2.5 border border-gray-300 rounded-xl">
                    <option value="">{{ __('all.all_schedules') }}</option>
                    @foreach ($schedules ?? [] as $schedule)
                        <option value="{{ $schedule->id }}"
                            {{ request('schedule_id') == $schedule->id ? 'selected' : '' }}>
                            {{ $schedule->activity }} - {{ $schedule->day }}
                        </option>
                    @endforeach
                </select>
                ...
                @if (request()->anyFilled(['branch_id', 'student_id', 'schedule_id', 'date', 'status']))
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500">{{ __('all.student_name') }}
                            </th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500">{{ __('all.nis') }}</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500">{{ __('all.schedule') }}</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-100">
                        @forelse($attendances as $attendance)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 font-medium">{{ $attendance->student->name ?? '-' }}</td>
                                <td class="px-6 py-4 text-sm">{{ $attendance->student->nis ?? '-' }}</td>
                                <td class="px-6 py-4 text-sm">{{ $attendance->schedule->activity ?? '-' }}</td>
                                <td class="px-6 py-4">
                                    <span
                                ...
                                <td class="px-6 py-4 text-right space-x-2">
                                    <a href="{{ route('attendance.student.show', $attendance) }}"
                                    ...
                                    <a href="{{ route('attendance.student.edit', $attendance) }}"
                                    ...
                                    <form id="delete-form-{{ $attendance->id }}"
                                        action="{{ route('attendance.student.destroy', $attendance) }}" method="POST"
                                        class="hidden">
                                        @csrf
                                        @method('DELETE')
                                    </form>
